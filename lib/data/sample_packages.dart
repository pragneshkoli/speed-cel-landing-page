...
class SamplePackages {
  ...
  static Future<Package?> getPackageByTrackingId(String trackingId) async {
    // http://localhost:3000/admin/track/order/S-250507-001
    // final response = await http.get(Uri.parse('http://localhost:3000/admin/track/order/$trackingId'));
    var request = http.Request('GET', Uri.parse('http://localhost:3000/admin/track/order/$trackingId'));

    http.StreamedResponse response = await request.send();
    String res = await response.stream.bytesToString();
    Map<String, dynamic> responseJson = jsonDecode(res);
    if (response.statusCode == 200) {
      Package package = Package.fromJson(responseJson['data']);
      return package;
    } else {
